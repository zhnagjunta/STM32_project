...
clock = time.clock()
red_threshold = (5,95,5,122,5,122)
black_threshold = (0, 17, -43, 3, -19, 6)
buf =[0 for i in range(5)]
buf1 =[0 for i in range(5)]
...
while(True):
        ...
        for i in range(0, 160, 20):
            ...
            if blobs:
                ...
                DI[k] = 0
        img = sensor.snapshot().lens_corr(strength = 1.8, zoom = 1.0)
        img.binary([black_threshold],invert = 1)
        img.erode(1, threshold = 4)
        m = -1
        for r in roi_1:
            m += 1
        ...
        if buf[0]==0 and buf[1] ==0 and buf[2] ==0 and buf[3] ==0 and buf[4] ==0: #尽头
            isten = 1
        if buf[0]==1 and (buf1[1] ==1 or buf1[4] ==1) : #障碍物
            isten = 2
